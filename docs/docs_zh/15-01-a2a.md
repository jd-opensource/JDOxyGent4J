...
│    ├── /.well-known/agent.json → AgentCard (SDK 类型)  │
...
```
oxygent-core/src/main/java/com/jd/oxygent/core/oxygent/samples/examples/a2a/
├── DemoA2AServer.java              — A2A 服务端
├── DemoA2AClient.java              — 非流式客户端
├── DemoA2AStreamClient.java        — 流式客户端
└── DemoA2ATaskFollowupClient.java  — 多轮对话客户端
```

## 互通性

OxyGent4J 的 A2A 实现与以下框架互通：
- Python OxyGent (`a2a-sdk`)
- 任何实现 A2A 0.3.x 协议的框架（LangChain、AgentScope、Google ADK 等）
